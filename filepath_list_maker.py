...
def filepaths(model): 
	...
	kit_storage6 = '/mnt/storage6/tahya/DFO/kit500_model_results/'
	...
	kit_subdirs = get_subdirs(kit_storage6)
	...
	#getting the list of paths using get_subdir_files
	if model=="grc100": filepaths = get_subdir_files(grc_subdirs)
	elif model=="kit500": filepaths = get_subdir_files(kit_subdirs)

	#file suffixes (at least, the ones we want)
	...
	for suffix in suffixes:
		filepaths_to_save = [filepath for filepath in filepaths if filepath.endswith(suffix)]
		with open('filepaths/'+model+'_filepaths_' + suffix[:-3] + '.csv', 'w', newline='') as output:
			write = csv.writer(output, quoting=csv.QUOTE_ALL)
			write.writerow(filepaths_to_save)

if __name__ == '__main__':
	for model in ['kit500','grc100']:
		filepaths(model)
